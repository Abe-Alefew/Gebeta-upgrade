...
const MenuItemDetail = () => {
        ...
        if (business && (business.id || business._id)) {
          ...
          if (response.success) {
            ...
            const formattedMenuItems = response.data.map(item => ({
              ...
              image: item.images?.find(img => img.isPrimary)?.url || item.images?.[0]?.url,
            ...
            if (menuItem) {
              const foundIndex = formattedMenuItems.findIndex(item => item.id === (menuItem._id || menuItem.id));
  ...
  useEffect(() => {
    const fetchItemReviews = async () => {
      ...
      try {
        ...
        const response = await reviewService.getByBusinessId(business._id || business.id);
        
        if (response.success) {
          // Filter reviews that belong to this specific menu item (if your backend supports it)
          // or show business reviews as a fallback
          const filteredReviews = response.data.filter(rev => rev.menuItem === currentItem.id);
          
          const formattedReviews = (filteredReviews.length > 0 ? filteredReviews : response.data).map(review => ({
            id: review._id,
            user: {
  ...
  */
  ...
  const smallImages = currentItem.allImages && currentItem.allImages.length > 0 
    ? currentItem.allImages.slice(0, 4) 
    : Array(4).fill(currentItem.image);

  const handleNextItem = () => setCurrentItemIndex((prev) => (prev + 1) % allMenuItems.length);
  const handlePrevItem = () => setCurrentItemIndex((prev) => (prev - 1 + allMenuItems.length) % allMenuItems.length);
  ...
  const renderStars = (rating) => {
    ...
    if (hasHalfStar) stars.push(<i key="half" className="fa-solid fa-star-half-stroke"></i>);
            ...
            <div className="modal-large-image-container">
              <img 
                ...
                onError={(e) => {
                  ...
                  e.target.src = 'https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?q=80&w=800';
                }}
              />
                ...
                {foodReviews.map((review, index) => (
                  <div key={review.id || index} className="modal-review-card">
                    <div className="modal-reviewer-info">
                      <img src={review.user?.avatar} alt={review.user?.name} className="modal-reviewer-avatar" />
